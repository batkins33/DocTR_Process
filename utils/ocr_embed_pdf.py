                ...
                for line in block.lines:
                    for word in line.words:
                        try:
                            ...
                            font_size = max(1, abs(y1 - y0) * 0.8)  # Scale font to box height
...
def run_ocr(input_path: str, force: bool = False) -> str:
    ...
    log.info(f"✅ Done: {len(doc)} pages in {elapsed:.1f}s ({elapsed/len(doc):.2f}s/page)")
    return out_path
...
def main():
    ...
    parser.add_argument("-o", "--output", help="Output PDF path (default: <input>_ocr.pdf)")

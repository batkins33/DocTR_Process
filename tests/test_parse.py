...
class TestFieldExtractor:
    ...
    def test_extract_fields_with_result(self):
        ...
        extraction_rules = {
            "test_vendor": {
                "ticket_number": {"pattern": r"TKT-(\d+)"}
            }
        }
        
        extractor = FieldExtractor(extraction_rules)
        
        with patch('doctr_process.parse.field_extractor.extract_vendor_fields') as mock_extract:
            mock_extract.return_value = {"ticket_number": "12345", "vendor": "test_vendor"}
...
class TestVendorDetector:
    ...
    def test_detect_vendor(self):
        ...
        vendor_rules = {
            "test_vendor": {"keywords": ["TEST", "VENDOR"]}
        }

...
class OutputManager:
    ...
    def _write_tickets_to_database(self, tickets: list[dict]):
        ...
        try:
            ...
            self.logger.info(
                f"Would write {len(tickets)} tickets to database (not yet implemented)"
            )
    ...
    def _write_csv_exports(self, tickets: list[dict], csv_dir: Path, suffix: str):
        ...
        if csv_config.get("manifest_log", True):
            contaminated_tickets = [
                t for t in tickets if t.get("material_class") == "CONTAMINATED"
            ]
    ...
    def _write_daily_summary_csv(self, tickets: list[dict], path: Path):
        ...
        try:
            ...
            daily_counts = defaultdict(
                lambda: {"total": 0, "by_material": defaultdict(int)}
            )
            ...
            with open(path, "w", newline="") as f:
                ...
                for date, counts in sorted(daily_counts.items()):
                    material_str = "; ".join(
                        [
                            f"{mat}: {count}"
                            for mat, count in counts["by_material"].items()
                        ]
                    )
    ...
    def write_review_queue(self, review_items: list[dict], suffix: str = ""):
        ...
        # Write to database if enabled
        if self.database_enabled and self.config["database"].get(

...
class ManifestValidator:
    ...
    def create_review_queue_entry(
        ...
        suggested_fixes = {
            "action": validation_result.suggested_action or "Manually enter manifest number from physical ticket",
            "material": validation_result.material_name,
            "requires_manifest": True,
    ...
    def get_manifest_statistics(
        ...
        contaminated_query = query.filter(
            TruckTicket.manifest_number.isnot(None)
        )
        ...
        review_query = self.session.query(ReviewQueue).filter(
            ReviewQueue.reason.like('%MISSING_MANIFEST%'),
        ...
        if start_date or end_date:
            ...
            if start_date:
                review_query = review_query.filter(TruckTicket.ticket_date >= start_date)
            if end_date:
        ...
        return {
            ...
            "recall_rate": 1.0 if missing_manifests == 0 else (tickets_with_manifests / total_requiring_manifests),

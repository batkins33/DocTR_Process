...
class ManifestNumberExtractor(BaseExtractor):
                ...
                if value:
                    ...
                    if "validation" in manifest_config:
                        is_valid, error = self.apply_validation(
                            value, manifest_config["validation"]
                        )
                        if not is_valid:
                            self.logger.warning(
    ...
    def is_required(self, material_type: str | None = None) -> bool:
        ...
        contaminated_materials = ["CLASS_2_CONTAMINATED", "CONTAMINATED", "REGULATED"]

...
class QuantityExtractor(BaseExtractor):
    ...
    def extract(self, text: str, **kwargs) -> tuple[float | None, str | None, float]:
    ...
    def _extract_quantity_with_unit(
        ...
        if value_str:
            try:
                ...
                for pattern_config in sorted(
                    patterns, key=lambda p: p.get("priority", 999)
                ):

...
from sqlalchemy import INT, VARCHAR, Boolean, DateTime, ForeignKey, Text
...
class ReviewQueue(Base):
    ...
    page_id: Mapped[str | None] = mapped_column(
        VARCHAR(255), nullable=True, comment="File path + page number identifier"
    )
    ...
    severity: Mapped[str] = mapped_column(
        VARCHAR(20), nullable=False, comment="Severity level (CRITICAL, WARNING, INFO)"
    )

    # File information
    file_path: Mapped[str | None] = mapped_column(
        VARCHAR(500), nullable=True, comment="Path to source file"
    )
    page_num: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Page number in file"
    )

    # JSON fields for context and suggestions
    detected_fields: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="JSON string of detected fields"
    )
    suggested_fixes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="JSON string of suggested fixes"
    )
    ...
    resolved_by: Mapped[str | None] = mapped_column(
        VARCHAR(100), nullable=True, comment="User who resolved the review item"
    )
    resolved_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="When the review item was resolved"
    )

    # Relationships
    ticket: Mapped[Optional["TruckTicket"]] = relationship(
        "TruckTicket", back_populates="review_entries"
    )
...
class ProcessingRun(Base):
    ...
    started_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="When processing started"
    )
    completed_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="When processing completed"
    )

    # Counts
    files_count: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of files processed"
    )
    pages_count: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of pages processed"
    )
    tickets_created: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of new tickets created"
    )
    tickets_updated: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of existing tickets updated"
    )
    duplicates_found: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of duplicate tickets found"
    )
    review_queue_count: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of items sent to review queue"
    )
    error_count: Mapped[int | None] = mapped_column(
        INT, nullable=True, comment="Number of errors encountered"
    )

    # Processing details
    processed_by: Mapped[str | None] = mapped_column(
        VARCHAR(100), nullable=True, comment="User/process that ran the batch"
    )
    ...
    config_snapshot: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="JSON snapshot of configuration used"
    )

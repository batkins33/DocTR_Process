...
def _validate_config_path(path: str, file_type: str) -> Path:
    ...
    try:
        ...
        )  # Go up from src/doctr_process/ocr/
        allowed_paths = [temp_dir, cwd, project_root]
    except (IndexError, OSError):
        allowed_paths = [temp_dir, cwd]

    if not any(
        str(path_obj).startswith(str(allowed_path)) for allowed_path in allowed_paths
    ):
    ...
    if path is None:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Cannot load extraction rules from {path}: {e}"
            ) from e
        except YAMLError as e:
            raise ValueError(
                f"Invalid YAML in extraction rules file {path}: {e}"
            ) from e
...
def load_config(config_path: str = None) -> dict:
    ...
    if config_path is None:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Cannot load config from {config_path}: {e}"
            ) from e
        except YAMLError as e:

...
class RefactoredPipeline:
    ...
    def setup_components(self):
        ...
        if vendor_csv_path:
            ...
            try:
                self.vendor_rules = load_vendor_rules_from_csv(
        ...
        self.input_handler = InputHandler(
            self.config.get("input_pdf") or self.config.get("input_dir")
        )
    ...
    def process_file(
        self, file_name: str, file_path: Path, file_data: bytes
    ) -> list[dict[str, Any]]:
        ...
        if not force_ocr and file_path and file_path.suffix.lower() == ".pdf":
            has_text, existing_text, page_text_status = (
                self.text_detector.check_pages_for_text(file_path)
            )
            if has_text and skip_ocr:
                logging.info(f"Using existing text from {file_name} (OCR skipped)")
                return self._process_existing_text(
                    file_name, existing_text, page_text_status
                )
        ...
        try:
            if file_data:
                images = list(
                    self.image_extractor.extract_from_bytes(file_data, file_name)
                )
            else:
        ...
        for i in range(0, len(images), batch_size):
            ...
            for j, ocr_result in enumerate(batch_results):
                ...
                # Detect vendor
                vendor_name, vendor_type, confidence, display_name = (
                    self.vendor_detector.detect_vendor(ocr_result["text"])
                )
    ...
    def _process_existing_text(
        self, file_name: str, text: str, page_text_status: list[bool] = None
    ) -> list[dict[str, Any]]:
        """Process file using existing extracted text instead of OCR."""
        # Detect vendor from existing text
        vendor_name, vendor_type, confidence, display_name = (
            self.vendor_detector.detect_vendor(text)
        )

        # Extract fields from text (simplified - no OCR result object)
        fields = dict.fromkeys(
            self.field_extractor.extraction_rules.get("DEFAULT", {}).keys()
        )
        ...
        if "ticket_number" in fields:
            ticket_match = re.search(
                r"(?:ticket|bol)\s*[:#]?\s*([A-Za-z0-9-]{5,})", text, re.IGNORECASE
            )
            if ticket_match:
                ...
                results.append(
                    {
                        ...
                        **fields,
                        ...
                        "field_issues": {},
                        "processing_method": (
                            "text_extraction" if had_text else "no_processing"
                        ),
    ...
    def run(self) -> None:
        ...
        if self.config.get("batch_mode"):
            ...
            for file_name, file_path, file_data in tqdm(
                self.input_handler.iter_file_contents(), desc="Processing files"
            ):
                try:
                    ...
                    # Add error record
                    all_results.append(
                        {"file": file_name, "page": 1, "vendor": None, "error": str(e)}
                    )
        else:
            # Process single file
        ...
        ocr_stats = (
            self.ocr_processor.get_stats()
            if hasattr(self.ocr_processor, "get_stats")
            else {}
        )

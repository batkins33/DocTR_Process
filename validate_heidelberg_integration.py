...
def test_extraction_rules():
    ...
    try:
        ...
        assert (
            "Heidelberg:" in content
        ), "Heidelberg rules not found in extraction_rules.yaml"
        ...
        assert (
            "method: text_regex" in content
        ), "text_regex method not found in Heidelberg rules"
...
def test_vendor_keywords():
    ...
    try:
        ...
        # Simple text-based checks
        assert (
            "Heidelberg,Heidelberg,Materials" in content
        ), "Heidelberg entry not found in vendor keywords"
        assert (
            "heidelberg" in content.lower()
        ), "Heidelberg match terms should include 'heidelberg'"
...
def test_regex_patterns():
    ...
    try:
        ...
        for field, expected_value in expected.items():
            ...
            assert (
                results[field] == expected_value
            ), f"Pattern for '{field}' extracted '{results[field]}', expected '{expected_value}'"
